...
- `gen_case_no_pins.py` — slimmer variant for boards **without** header pins
  ...
  and `esp32c3_supermini_case_slim_lid.stl`. Total assembled height ~12.5mm.
...
| `LID_LIP_H` | 4.0 mm | lip depth — taller than strictly needed, so the ridge/groove band (see below) stays a localized notch instead of spanning the whole lip |
